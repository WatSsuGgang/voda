...
### Nginx 설치
* React 빌드 파일을 공유하기 위한 도커 볼륨 생성
```sh
docker volume create html

docker volume inspect html # 생성된 볼륨에 대한 정보를 확인
```
* 서버에서 Nginx 설정을 해주고, 설정 파일이 있는 폴더 및 경로를 도커 볼륨을 통해 컨테이너에서도 접근 가능하게 설정
...
* http 접속을 위한 80 포트와 https 접속을 위한 443 포트를 열어줌
```sh
docker pull nginx
...
### Jenkins 설치
* 프론트 빌드 결과를 Nginx 컨테이너와 공유하기 위해 html 볼륨을 연결해놓음
* Jenkins 컨테이너는 도커 설치가 안 돼 있기 때문에 docker.sock을 볼륨으로 연결해서 도커 명령어를 사용
...
* nginx 설정 파일인 /etc/nginx/conf.d/default.conf
  * root와 index를 위에서 설정한 볼륨 정보에 맞게 설정
    * Nginx가 정적 파일을 서빙하도록 설정
  * http로 요청이 오면, https로 리다이렉트
  * [Vite Pwa Nginx 배포 가이드라인](https://vite-pwa-org.netlify.app/deployment/nginx)을 참고하여 추가적인 설정 필요
  * /api/v1/ uri에 대한 리버스 프록시 설정
```nginx
server {
    listen 80;
...
## Jenkins Pipeline 설정
* [젠킨스 깃랩 연결 가이드](https://docs.gitlab.com/ee/integration/jenkins.html)를 참고하여 깃랩과 젠킨스를 연결
  * 깃랩의 웹훅 - 젠킨스의 파이프라인
...
```
* 컨테이너를 실행할 때, 네트워크를 연결하고 실행해 컨테이너끼리 통신이 가능하도록 설정
```sh
...
* Nginx 컨테이너와 실행중인 컨테이너가 같은 도커 네트워크에 속해 있다면, 컨테이너 이름으로 리버스 프록시 설정이 가능함
```nginx
server {
  listen 80;
  server_name {SERVER_URL};
...
* 도커 실행 시 `-p`를 사용하면 ufw 적용이 제대로 되지 않음
  * `-p`는 iptable을 직접 수정하기 때문에 ufw status에 반영되지 않음
* 해결책
  * `-p`를 사용하지 말 것. 대신 docker network를 사용
  * 컨테이너를 로컬호스트로 실행해 호스트 머신 외부로 노출시키지 말 것
